...
let pointer_of_derefed_slice_to_slice files =
  (object
     ...
     method! visit_typ () t =
       match t with
       | TBuf (TApp (lid, [ t ]), const) when lid = Builtin.derefed_slice ->
           super#visit_typ () (Builtin.mk_slice ~const t)
       | TBuf (TQualified lid, const) when Krml.MonomorphizationState.essentially_slice lid ->
           super#visit_typ () (Builtin.mk_slice ~const (TQualified lid))
       | _ -> super#visit_typ () t
  end)
    #visit_files
    () files

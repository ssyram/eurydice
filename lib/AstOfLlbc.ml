...
let rec expression_of_fn_ptr env depth (fn_ptr : C.fn_ptr) =
  let {
    C.generics = { types = type_args; const_generics = const_generic_args; _ };
    kind;
    _;
  } : C.fn_ptr =
    fn_ptr
...
let file_of_crate (crate : Charon.LlbcAst.crate) : Krml.Ast.file =
  ...
  let get_nth_function id =
    ...
    | Some f -> f
    | None -> fail "Function id not found: %s" (Charon.PrintExpressions.fun_decl_id_to_string format_env id)
  in
  let get_nth_type id =
  ...
  let get_nth_global id =
    ...
    | Some g -> g
    | None -> fail "Global id not found: %s" (Charon.PrintTypes.global_decl_id_to_string format_env id)
  in
  let get_nth_trait_impl id =
    match C.TraitImplId.Map.find_opt id trait_impls with
    | Some impl -> impl
    | None -> fail "Trait impl id not found: %s" (Charon.PrintTypes.trait_impl_id_to_string format_env id)
  in
  let get_nth_trait_decl id =
    match C.TraitDeclId.Map.find_opt id trait_decls with
    | Some decl -> decl
    | None -> fail "Trait decl id not found: %s" (Charon.PrintTypes.trait_decl_id_to_string format_env id)
  in

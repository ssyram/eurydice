...
let lookup_fun (env : env) depth (f : C.fn_ptr) : K.expr' * lookup_result * C.trait_ref list =
  ...
  (* Enhanced matching for const generics: use re_polymorphize approach *)
  let matches_with_repolymorphize pattern =
    if matches pattern then true
    else
      (* Try with re_polymorphize approach for const generics *)
      let fn_ptr_string = string_of_fn_ptr env f in
      let pattern_string = string_of_pattern pattern in
      L.log "Calls" "%sTrying re_polymorphize matching: %s vs %s" depth fn_ptr_string pattern_string;
      (* Simple heuristic: if the function has const generics, try relaxed matching *)
      String.contains pattern_string '@' && 
      (let base_pattern = Str.global_replace (Str.regexp "@[A-Za-z0-9_]*") ".*" pattern_string in
       let regex = Str.regexp base_pattern in
       try Str.string_match regex fn_ptr_string 0 
       with _ -> false)
  in

  (* Try pattern matching with enhanced const generics support *)
  match List.find_opt (fun (p, _) -> matches_with_repolymorphize p) known_builtins with
